@extends('layouts.app')

@section('title', 'edite jardin')

@section('content')

<form action="{{ route('jardins.update', $exactjardin->id)}}" method="POST">
@csrf
@method('PUT')
<h2>{{ __('messages.updateTitle')}}</h2>
<br><div>
    <label for="inputAddress">{{ __('messages.jardinName')}}</label>
    <input type="text" name="name" value="{{ $exactjardin->name}}" required>
</div>
<br><div>
<label for="inputAddress">{{ __('messages.jardinEspace')}}</label>
<input type="number" name="espace" value="{{ $exactjardin->espace}}"  required>
<br>
<label for="inputAddress">{{ __('messages.jardinjardinier')}}</label>
<select name="jardinier_id" id="">
    @foreach ($alljardiniers as $jardinier )
    <option value="{{$jardinier->id}}">{{ $jardinier->name}}</option>
        
    @endforeach
</select>
<br><button type="submit">{{'messages.createBtn'}}</button>

</div>
</form>